...
public class TabFragment3 extends Fragment {
    ...
    ArrayList<ImageUploadInfo> employeeFeed = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mAdapter = new MoviesAdapter(employeeFeed, this.getActivity());
            ...
            if(dataFeed.size()>0) {
                for (int i = 0; i < dataFeed.size(); i++) {
                    if (dataFeed.get(i).getImageUploadInfo().getStatus().equalsIgnoreCase("Approved") &&
                            dataFeed.get(i).getImageUploadInfo().getPostTo().equalsIgnoreCase("Administration")) {
                        employeeFeed.add(dataFeed.get(i).getImageUploadInfo());
                    }
                }
            }
        }
